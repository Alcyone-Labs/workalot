...
class BenchmarkCLI {
  ...
  async run(args: string[]): Promise<void> {
    ...
    configsToRun.forEach((config, index) => {
      const difficultyStr = config.difficulty && config.difficulty !== 1.0
        ? ` [${config.difficulty}x difficulty]`
        : '';
      console.log(`${index + 1}. ${config.name} (${config.cores} cores, ${config.totalJobs.toLocaleString()} jobs)${difficultyStr}`);
    });
    console.log('');
    ...
    for (let i = 0; i < configsToRun.length; i++) {
      ...
      try {
        ...
        console.log(`   👥 Job distribution: ${jobDistribution} (${avgUtilization.toFixed(1)}% avg utilization)`);
      ...
      if (i < configsToRun.length - 1) {
        ...
        logger.info('BENCHMARK', `Completed benchmark ${i + 1}/${configsToRun.length}, cooling down before next benchmark`, {
    ...
    logger.info('BENCHMARK', `All benchmarks completed`, { totalBenchmarks: configsToRun.length, successfulBenchmarks: results.length });
   ...
   */
  private selectConfigs(configNames?: string[], difficulty?: number, runAll?: boolean): BenchmarkConfig[] {
...
`);
  ...
  private showSummary(results: any[]): void {
    ...
    results.forEach(result => {
      const jobDistribution = result.executionPhase.workerStats.map((w: any) => w.jobsProcessed).join(':');
      const avgUtilization = result.executionPhase.workerStats.reduce((sum: number, w: any) => sum + w.utilizationPercentage, 0) / result.executionPhase.workerStats.length;

      console.log(`\n${result.config.name}:`);
      console.log(`  Throughput: ${result.jobsPerSecond.toFixed(2)} jobs/sec`);
      console.log(`  Total time: ${(result.totalTime / 1000).toFixed(2)}s`);
      console.log(`  Peak CPU: ${result.executionPhase.peakCPU.toFixed(1)}%`);
      console.log(`  Peak Memory: ${(result.executionPhase.peakMemory / 1024 / 1024).toFixed(1)} MB`);
