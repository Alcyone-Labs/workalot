...
/**
 * Telemetry hook interface for external observability integration
 */
export interface TelemetryHooks {
  /** Called when a job is scheduled */
  onJobScheduled?: (jobId: string, payload: JobPayload) => void;
  /** Called when a job starts executing */
  onJobStarted?: (jobId: string, workerId?: number) => void;
  /** Called when a job completes successfully */
  onJobCompleted?: (jobId: string, result: JobResult, payload: JobPayload) => void;
  /** Called when a job fails */
  onJobFailed?: (jobId: string, error: string, payload: JobPayload) => void;
  /** Called periodically with queue statistics */
  onQueueStats?: (stats: { pending: number; processing: number; completed: number; failed: number }) => void;
  /** Called periodically with worker statistics */
  onWorkerStats?: (workers: { total: number; busy: number; available: number }) => void;
}
...
export class TaskManager extends EventEmitter {
  ...
  private jobPayloads = new Map<string, JobPayload>(); // Track payloads for telemetry
  ...
  /**
   * Register telemetry hooks for observability integration
   */
  registerTelemetryHooks(hooks: TelemetryHooks): void {
   ...
   */
  unregisterTelemetryHooks(): void {
    this.telemetryHooks = undefined;
  }
  ...
  private setupEventHandlers(): void {
    ...
    this.jobScheduler.on('job-scheduled', (jobId: string, payload: JobPayload) => {
      this.emit('job-scheduled', jobId);
      // Store payload for later retrieval on completion
      if (this.telemetryHooks) {
        this.jobPayloads.set(jobId, payload);
        this.telemetryHooks.onJobScheduled?.(jobId, payload);
      }
    });

    this.jobScheduler.on('job-started', (jobId: string, workerId: number) => {
      this.telemetryHooks?.onJobStarted?.(jobId, workerId);
    });

    this.jobScheduler.on('job-completed', (jobId: string, result: JobResult) => {
      this.emit('job-completed', jobId, result);
      // Retrieve payload and clean up
      const payload = this.jobPayloads.get(jobId);
      if (payload) {
        this.telemetryHooks?.onJobCompleted?.(jobId, result, payload);
    ...
    this.jobScheduler.on('job-failed', (jobId: string, error: string) => {
      ...
      const payload = this.jobPayloads.get(jobId);
      if (payload) {
        this.telemetryHooks?.onJobFailed?.(jobId, error, payload);
        this.jobPayloads.delete(jobId);
      }
      this.checkAndCallWhenFreeCallbacks();
    });
    ...
    if (this.telemetryHooks?.onQueueStats || this.telemetryHooks?.onWorkerStats) {
      setInterval(async () => {
        if (!this.isInitialized) return;
        
        try {
          const queueStats = await this.queueManager.getStats();
          this.telemetryHooks?.onQueueStats?.({
          ...
          const schedulerStats = await this.jobScheduler.getStats();
          if (schedulerStats.workers) {
            this.telemetryHooks?.onWorkerStats?.({
              total: schedulerStats.workers.total || 0,
              busy: schedulerStats.workers.busy || 0,
              available: schedulerStats.workers.available || 0
            });
          }
        } catch (error) {
