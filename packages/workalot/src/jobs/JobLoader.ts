...
import { IJob, JobPayload, JobExecutionContext } from "../types/index.js";
...
export class JobLoader {
  ...
  private validateJob(job: any, jobFile: string): asserts job is IJob {
    if (!job || typeof job !== "object") {
      throw new JobValidationError("Job must be an object instance", jobFile);
    }
    ...
    if (typeof job.run !== "function") {
      throw new JobValidationError("Job must implement run() method", jobFile);
    }
  }
  ...
  async executeJob(
    ...
    try {
      const result = await job.run(jobPayload.jobPayload, context);
      return result;
    } catch (error) {
