...
import { initializeTaskManager, scheduleAndWait, getQueueStats, shutdown } from '../src/index.js';
...
export class GreetingJob extends BaseJob {
  ...
  async run(payload: { name: string; language: string }): Promise<Record<string, any>> {
    ...
    const greeting = greetings[payload.language as keyof typeof greetings] || greetings.en;
...
async function demonstrateSQLiteBackend() {
  ...
  const results = await Promise.all(
    jobs.map(job => scheduleAndWait({
      jobFile: './examples/sqlite-backend.ts',  // Use this file as the job source
      jobPayload: job
    }))
  );
  ...
  console.log(`📊 Throughput: ${Math.round(jobs.length / (duration / 1000))} jobs/sec`);

  // Show results
  console.log('\n📝 Job Results:');
  results.forEach((result: any, index: number) => {
    console.log(`   ${index + 1}. ${result.results.data.message} (${result.executionTime}ms)`);
  });
  ...
  console.log(`💾 Scheduling ${persistentJobs.length} jobs with persistence...`);
  
  const persistentResults = await Promise.all(
    persistentJobs.map(job => scheduleAndWait({
      jobFile: './examples/sqlite-backend.ts',
      jobPayload: job
    }))
  );
  ...
  console.log(`   Total: ${finalStats.total}, Completed: ${finalStats.completed}`);
  ...
  console.log('\n💡 Key Benefits:');
  console.log('   • Runtime-optimized drivers (Bun native SQLite + better-sqlite3 fallback)');
