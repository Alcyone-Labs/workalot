...
async function performanceTest() {
  ...
  for (const config of configs) {
    ...
    for (let i = 0; i < config.jobs; i++) {
      jobPromises.push(
        taskManager.scheduleAndWait({
          jobFile: 'examples/performance-test.ts',
          jobPayload: { 
    ...
    const avgExecutionTime = results.reduce((sum: number, r: any) => sum + r.executionTime, 0) / results.length;
  ...
  for (const backend of backends) {
    ...
    try {
      ...
      const jobs = Array.from({ length: jobCount }, (_, i) =>
        taskManager.scheduleAndWait({
          jobFile: 'examples/performance-test.ts',
          jobPayload: { iterations: 1000, jobIndex: i, timestamp: Date.now(), random: Math.random() }
        })
      );
...
if (process.argv[1] === import.meta.url.replace('file://', '')) {
  ...
  if (args.includes('--backends') || args.includes('-b')) {
    ...
    performanceTest().then(() => backendComparison()).catch(console.error);
  } else {
